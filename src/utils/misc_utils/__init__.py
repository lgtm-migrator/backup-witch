    ...
    for line in file:
        if "ERROR" in line and "Can't retry any of the errors" not in line:
            ...
            if all(check(line, file) for check in checks):
                return True
    if (
        no_errors_encountered
    ):  # todo remove this? as there are errors, which don't have 'ERROR' in them
        raise Unexpected(
            "Rclone log was checked for not ignored errors but no errors were encountered"
        )
    return False
